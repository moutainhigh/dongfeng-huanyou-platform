...
//			e.printStackTrace();
	...
	public static int bytesToInt(byte[] bytes){
	    ...
	    if(bytes.length == 2){
		    if(bytes[1] < 0 ){cc[1] = (long)(bytes[1] & 0x7f) + 0x80;}else{cc[1] = bytes[1];}
		    if(bytes[0] < 0 ){cc[0] = (long)((bytes[0] & 0x7f)<<8) + 0x8000;}else{cc[0] = bytes[0]<<8;}
		    int res = (int)( cc[1] + cc[0]);
		    return res;
	    }else if(bytes.length == 4){
		    if(bytes[3] < 0 ){cc[3] = (long)(bytes[3] & 0x7f) + 0x80;}else{cc[3] = bytes[3];}
		    if(bytes[2] < 0 ){cc[2] = (long)((bytes[2] & 0x7f)<<8) + 0x8000;}else{cc[2] = bytes[2]<<8;}
		    if(bytes[1] < 0 ){ cc[1] = (long)((bytes[1] & 0x7f)<<16) + 0x800000;}else{cc[1] = bytes[1]<<16;}
		    if(bytes[0] < 0 ){ cc[0] = (long)((bytes[0] & 0x7f)<<24) + 0x80000000;}else{cc[0] = bytes[0]<<24;}
	 ...
	 */
	public static final byte[] getNewBytes(final byte[] bytes,
