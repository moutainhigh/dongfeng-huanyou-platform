...
/**
 * 采集执行标准版本应答 Gps_3055_TRVersionRes
 * 
 * @author maojin
 * 
 */
@Component
public class Gps_3055_TRVersionRes extends Command {
...
//			
//			TRVersionRes res=TRVersionRes.parseFrom(packet.getContentForBytes());
...
//			if(null==oldPacket){
			...
			sendMsgToClient(packet, "行驶记录仪--采集执行标准版本应答");
//			}else{//809发送的
//					packet.setCommand("5115");
//					packet.addParameter("oldCommand", "3055");
//				GpsCommandFactory.processor(packet);
//			}

		} catch (Exception e) {
			log.error("行驶记录仪--采集执行标准版本应答异常", e);
		}
		return null;
	}
