...
import com.navinfo.opentsp.dongfeng.common.sqllaber.hibernateOverride.MappingReader;
...
public class SqlLaberUtil implements ISqlLaberUtil, ResourceLoaderAware, InitializingBean
{
    ...
    @Value("${sql.xml.files.path:classpath*:/dynamicsql/*-dynamic.xml}")
    ...
    private void initTemplate()
    {
        ...
        for (Entry<String, String> entry : namedHQLQueries.entrySet())
        {
            ...
            {
                LOGGER.error("HQL配置文件加载异常：sqlname:" + entry.getKey() + "--加载异常，请检查语法",e);
            }
        }
        for (Entry<String, String> entry : namedSQLQueries.entrySet())
        {
            ...
            {
                LOGGER.error("SQL配置文件加载异常：sqlname:" + entry.getKey() + "--加载异常，请检查语法",e);
            }
        }
    }
