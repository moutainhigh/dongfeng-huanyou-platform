package com.navinfo.opentsp.dongfeng.common.dao.impl;

import com.navinfo.opentsp.dongfeng.common.dao.IRedisDao;
...
public class RedisDao implements IRedisDao {
    ...
    @Override
    public Set<byte[]> sMembersForBytes(final String key) {
        return instance.execute(new RedisCallback<Set<byte[]>>() {
            @Override
            public Set<byte[]> doInRedis(RedisConnection redisConnection) throws DataAccessException {
                Set<byte[]> bytes = redisConnection.sMembers(getBytes(key));
    ...
    @Override
    public List<Object> hMGetListALL(final Map<String, String> keys) {
        List ret = (List)this.instance.execute(new RedisCallback() {
            @Override
            public List<Object> doInRedis(RedisConnection redisConnection) throws DataAccessException {
                redisConnection.openPipeline();
                if(keys != null) {
                    Iterator list = keys.entrySet().iterator();
                    while (list.hasNext()) {
                        Map.Entry entry = (Map.Entry) list.next();
                        byte[] keyBytes = getBytes(String.valueOf(entry.getKey()));
                        byte[] hashKey = getBytes(String.valueOf(entry.getValue()));
                        redisConnection.hMGet(keyBytes, hashKey);
                    }
                }
                List<Object> var11 = redisConnection.closePipeline();
                return var11;
            }
        });
        return ret;
