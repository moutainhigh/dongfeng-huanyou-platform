    ...
    public static void deleteJpg(String dir)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < files.length; i++)
            {
                ...
                fileName = f.getName();
                if (fileName.endsWith(VerifyCodeConstants.VERIFYCODE_EXTENSION_WITH_DOT))
                {
                    f.delete();// 删除后缀名为jpg的文件
                    logger.debug("删除临时验证码文件失败" + fileName + "成功！");
                }
            }
        }
        catch (Exception e)
        {
            logger.error("删除临时验证码文件失败" + fileName + "失败！" , e);
        }
    }
