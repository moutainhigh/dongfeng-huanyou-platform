package com.navinfo.opentsp.dongfeng.common.sqllaber;
...
/**
 * hibernate动态sql dtd解析器用来校验文件格式
 * 
 * @author zhangy
 *
 */
public class DtdParse implements EntityResolver, Serializable
{
    private static final long serialVersionUID = 8123799007554762965L;
    
    private static final Logger LOGGER = Logger.getLogger(DtdParse.class);
    
    private static final String HOP_DYNAMIC_STATEMENT = "http://127.0.0.1/dtd/dynamic-hibernate-statement-1.0.dtd/";
    ...
    private InputSource resolveOnClassPath(String publicId, String systemId, String namespace)
    {
        ...
        if (dtdStream == null)
        {
            ...
            {
                LOGGER.info("Don't use old DTDs, read the Hibernate 3.x Migration Guide!");
            }
        }
        else
        {
