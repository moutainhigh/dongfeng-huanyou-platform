...
public enum ReturnCode implements InterfaceResultCode {
    ...
    ROOT_TEAM_CANNOT_DEL(650101, "不允许删除根节点"), DEALER_ALREADY_EXISTS(650102, "经销商代码重复"), ROOT_CAR_TEAM_CANNOT_SAVEORUPDATE(
            650103, "不允许新增或更改根节点"), DEALERNAME_ALREADY_EXISTS(650104, "经销商名称重复"), DEALER_CITYANDPROVICE_NOZERO(650105, "省市编码不能为空或者0"),
    SING_SYSTEM_ERROR(650106, "标签系统服务端异常"),
    // 用户鉴权相关
    ...
    EXISTED_TERMINAL_ID(650501, "终端ID已存在"), EXISTED_COMMUNICATION_ID(650502, "终端通讯号已存在"), EXISTED_DEVICE_LABEL_ID(
            ...
            "终端SIM卡已存在"), CAN_NOT_UPDATE_TEAM(650531, "终端已和车辆绑定,不能修改所属经销商"), TERMINAL_RECOVER(650532, "恢复删除终端"), EXPORT_TERMINAL_FAILED(65053, "导出终端失败"),
    QUERY_CAN_BUS_FAILED(65053, "查询CAN总线数据失败"), REAL_TIME_DATA_COLLECT_SETTING_FAILED(65054, "实时数据采集汇报设置"), SPECIAL_DATA_COLLECT_SETTING_FAILED(65055, "特殊数据采集汇报设置"),
    UNKNOWN_ERROR(650599, "未知异常"), COMMUNICATION_ID_ALREADY_EXIST(
    ...
    STATION_SYNC_TO_TAG_FAILED(650620, "同步服务站信息到同步电子标签系统失败"), STATION_SYNC_FAILED(650621, "同步服务站失败"),
    // 目录查询
    QUERY_MENU_FAILED(650701, "获取目录异常"),
    //车辆管理
    QUERY_NON_F9_VEHICLE_FAILED(650801, "查询非F9车辆失败"), UPDATE_NON_F9_VEHICLE_FAILED(650802, "更新非F9车辆失败"), EXPORT_NON_F9_VEHICLE_FAILED(650803, "导出非F9车辆失败"),
    CHASSIS_NUM_EXIST(650804, "底盘号已经存在"),VIN_NUM_EXIST(650805, "车架号已经存在"),
    // 消息推送
    ...
    ROLE_MODIFY(680807, "角色下已经有相应的用户类型,不可以修改该角色类型"), ROLE_DELETE(680808, "角色下用户仅有一个,无法删除角色"),
    // 服务配件预约
    ...
    // 监控模块
    TIME_BEGIN_AND_END_CHECK(800801, "开始时间不能大于结束时间"),TIMELY_DATA_INTERFACE_ERROR(800802, "调用实时数据查询接口异常"),
    // 操作日志
    ...
    INVOICE_NUMBER_AREADY_EXIST(851001,
            ...
            "数据为空,导入空入车辆扫码信息失败"), IMPORT_SCANREPORT_DUPLICATE(851004, "数据重复导入"),
    ...
    IMPORT_SCANCODE_SYNC_SWITCH_OFF(670209, "同步开关未开启，请联系管理员"), IMPORT_SCANCODE_SYNC_DTS_FAILED(670210, "同步失败"),
    IMPORT_SCANCODE_SYNC_DTS_FAILED_DATA_EMPTY(670211, "车辆同步信息不存在"),
    EXPORT_FILE_SUCCESS(670212, "文件下载成功"),

    AUDIT_FAILED(670213, "审核失败"),
    /*************************************************** 业务错误提示 End ************************************************************************/
