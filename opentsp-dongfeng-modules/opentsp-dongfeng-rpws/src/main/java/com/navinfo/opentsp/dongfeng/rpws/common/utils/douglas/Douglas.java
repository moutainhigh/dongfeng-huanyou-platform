...
public class Douglas {
	...
	/************
	 * 采集点读取
	 * @param points 格式：1 4,2 3,4 2 (x坐标y坐标以空格相连，多点以逗号相连)
	 */
	public void readPoint(String pointStr) {
	 ...
	 * @param points 格式：1 4,2 3,4 2 (x坐标y坐标以空格相连，多点以逗号相连)
	 */
	public void readPoint(List<Point> pointsp) {
		 points = pointsp;
	}
...
//		System.out.println(to.getY());
	 ...
	 */
	public void compress(Point from, Point to,double precision) {
		/****抽稀精度设置******/
		D=precision;
...
//		System.out.println();
		...
		d.readPoint("1 4,2 3,4 2,6 6,7 7,8 6,9 5,10 10");
		d.compress(d.points.get(0), d.points.get(d.points.size() - 1),3);
		for (int i = 0; i < d.points.size(); i++) {
			Point p = d.points.get(i);
			if (p.getIndex() > -1) {
//				System.out.print(p.getX() + "——" + p.getY() + ",");
