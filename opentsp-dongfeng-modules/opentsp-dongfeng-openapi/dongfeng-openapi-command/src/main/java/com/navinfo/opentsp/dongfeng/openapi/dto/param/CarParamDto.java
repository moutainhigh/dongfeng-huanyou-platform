package com.navinfo.opentsp.dongfeng.openapi.dto.param;

/**
 * 有关一汽对应的车型数据类
 * 
 * @author xltianc.zh
 *
 */
public class CarParamDto {

	private Long id;
	private String lcxm; // 老车型码
	private String cxm;// 车型码
	private String sjxh;// 设计型号
	private String gg;// 公告
	private String cx;// 车型
	private String jss;// 驾驶室
	private String fdj;// 发动机
	private String bsx;// 变速箱
	private String lhq;// 离合器
	private String bjsj;// 编辑时间
	private String qz;// 前轴
	private String hq;// 后桥
	private String cj;// 车架
	private String cxg;// 车厢
	private String colorCode;// 颜色码
	private String jgl;// 净功率
	private String edgl;// 额定功率
	private String color;// 颜色
	private String zj;// 轴距
	private String fa;// 方案
	private String lt;// 轮胎
	private String qt;// 其他

	public Long getId() {
		return id;
	}

	public void setId(Long id) {
		this.id = id;
	}

	public String getLcxm() {
		return lcxm;
	}

	public void setLcxm(String lcxm) {
		this.lcxm = lcxm;
	}

	public String getCxm() {
		return cxm;
	}

	public void setCxm(String cxm) {
		this.cxm = cxm;
	}

	public String getSjxh() {
		return sjxh;
	}

	public void setSjxh(String sjxh) {
		this.sjxh = sjxh;
	}

	public String getGg() {
		return gg;
	}

	public void setGg(String gg) {
		this.gg = gg;
	}

	public String getCx() {
		return cx;
	}

	public void setCx(String cx) {
		this.cx = cx;
	}

	public String getJss() {
		return jss;
	}

	public void setJss(String jss) {
		this.jss = jss;
	}

	public String getFdj() {
		return fdj;
	}

	public void setFdj(String fdj) {
		this.fdj = fdj;
	}

	public String getBsx() {
		return bsx;
	}

	public void setBsx(String bsx) {
		this.bsx = bsx;
	}

	public String getLhq() {
		return lhq;
	}

	public void setLhq(String lhq) {
		this.lhq = lhq;
	}

	public String getBjsj() {
		return bjsj;
	}

	public void setBjsj(String bjsj) {
		this.bjsj = bjsj;
	}

	public String getQz() {
		return qz;
	}

	public void setQz(String qz) {
		this.qz = qz;
	}

	public String getHq() {
		return hq;
	}

	public void setHq(String hq) {
		this.hq = hq;
	}

	public String getCj() {
		return cj;
	}

	public void setCj(String cj) {
		this.cj = cj;
	}

	public String getCxg() {
		return cxg;
	}

	public void setCxg(String cxg) {
		this.cxg = cxg;
	}

	public String getColorCode() {
		return colorCode;
	}

	public void setColorCode(String colorCode) {
		this.colorCode = colorCode;
	}

	public String getJgl() {
		return jgl;
	}

	public void setJgl(String jgl) {
		this.jgl = jgl;
	}

	public String getEdgl() {
		return edgl;
	}

	public void setEdgl(String edgl) {
		this.edgl = edgl;
	}

	public String getColor() {
		return color;
	}

	public void setColor(String color) {
		this.color = color;
	}

	public String getZj() {
		return zj;
	}

	public void setZj(String zj) {
		this.zj = zj;
	}

	public String getFa() {
		return fa;
	}

	public void setFa(String fa) {
		this.fa = fa;
	}

	public String getLt() {
		return lt;
	}

	public void setLt(String lt) {
		this.lt = lt;
	}

	public String getQt() {
		return qt;
	}

	public void setQt(String qt) {
		this.qt = qt;
	}

}
