...
public class WebController
{
    ...
    @Autowired
    private AppContriller appContriller;
    ...
    {
        // app接口f9ReportInfo 兼容app接口用
        if (StringUtil.isEq(uri, "/dongfeng/openapi/apps_LastGpsByCloud"))
        {
            long startTime = System.currentTimeMillis();
            F9ReportInfoCommand cmd = JsonUtil.fromJson(param, F9ReportInfoCommand.class);
            long endTime = System.currentTimeMillis();
            float seconds = (endTime - startTime) / 1000F;
            logger.info(uri + " request is:" + Float.toString(seconds) + " seconds.");
            return appContriller.f9ReportInfo(cmd);
        }
        // app 兼容app接口用
        if (StringUtil.isEq(uri, "/dongfeng/openapi/apps_LastTerminalStatue"))
        {
            long startTime = System.currentTimeMillis();
            F9ScanResultCommand cmd = JsonUtil.fromJson(param, F9ScanResultCommand.class);
            long endTime = System.currentTimeMillis();
            float seconds = (endTime - startTime) / 1000F;
            logger.info(uri + " request is:" + Float.toString(seconds) + " seconds.");
            return appContriller.f9ScanResultInfo(cmd);
        }
